...
@Controller('api/dataradiologi')
export class RadiologiController {
  constructor(private readonly service: RadiologiService) {}
  ...
  @Post()
  @UseInterceptors(
    FileInterceptor('gambarradiologi', {
      storage: diskStorage({
        destination: join(process.cwd(), 'uploads/radiologi'),
  ...
  async create(@Body() data: any, @UploadedFile() file: any) {
    try {
      if (file) {
        const filePath = join(process.cwd(), 'uploads/radiologi', file.filename);
      ...
      throw new BadRequestException('Failed to save radiologi data: ' + error.message);
    }
  }
  ...
  async update(@Param('id') id: string, @Body() data: any, @UploadedFile() file: any) {
    try {
      ...
      throw new BadRequestException('Failed to update radiologi data: ' + error.message);
    }
  }
  ...
  async remove(@Param('id') id: string) {
    try {
      ...
      throw new BadRequestException('Failed to remove radiologi data: ' + error.message);
    }
  }
}
