...
@Controller('api/datamcu')
export class McuController {
  constructor(private readonly service: McuService) {}
  ...
  @Post()
  @UseInterceptors(
    FileInterceptor('gambarmcu', {
      storage: diskStorage({
        destination: join(process.cwd(), 'uploads/mcu'),
  ...
  async create(@Body() data: any, @UploadedFile() file: any) {
    try {
      if (file) {
        const filePath = join(process.cwd(), 'uploads/mcu', file.filename);
      ...
      throw new BadRequestException('Failed to save mcu data: ' + error.message);
    }
  }
  ...
  async update(@Param('id') id: string, @Body() data: any, @UploadedFile() file: any) {
    try {
      ...
      throw new BadRequestException('Failed to update mcu data: ' + error.message);
    }
  }
  ...
  async remove(@Param('id') id: string) {
    try {
      ...
      throw new BadRequestException('Failed to remove mcu data: ' + error.message);
    }
  }
}
