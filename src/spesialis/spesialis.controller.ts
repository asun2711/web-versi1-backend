...
@Controller('api/dataspesialis')
export class SpesialisController {
  constructor(private readonly service: SpesialisService) {}
  ...
  @Post()
  @UseInterceptors(
    FileInterceptor('gambarspesialis', {
      storage: diskStorage({
        destination: join(process.cwd(), 'uploads/spesialis'),
  ...
  async create(@Body() data: any, @UploadedFile() file: any) {
    try {
      if (file) {
        const filePath = join(process.cwd(), 'uploads/spesialis', file.filename);
      ...
      throw new BadRequestException('Failed to save spesialis data: ' + error.message);
    }
  }
  ...
  async update(@Param('id') id: string, @Body() data: any, @UploadedFile() file: any) {
    try {
      ...
      throw new BadRequestException('Failed to update spesialis data: ' + error.message);
    }
  }
  ...
  async remove(@Param('id') id: string) {
    try {
      ...
      throw new BadRequestException('Failed to remove spesialis data: ' + error.message);
    }
  }
}
