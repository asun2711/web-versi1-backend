...
@Controller('api/datapenghargaan')
export class PenghargaanController {
  constructor(private readonly service: PenghargaanService) {}
  ...
  @Post()
  @UseInterceptors(
    FileInterceptor('gambarpenghargaan', {
      storage: diskStorage({
        destination: join(process.cwd(), 'uploads/penghargaan'),
  ...
  async create(@Body() data: any, @UploadedFile() file: any) {
    try {
      if (file) {
        const filePath = join(process.cwd(), 'uploads/penghargaan', file.filename);
      ...
      throw new BadRequestException('Failed to save penghargaan data: ' + error.message);
    }
  }
  ...
  async update(@Param('id') id: string, @Body() data: any, @UploadedFile() file: any) {
    try {
      ...
      throw new BadRequestException('Failed to update penghargaan data: ' + error.message);
    }
  }
  ...
  async remove(@Param('id') id: string) {
    try {
      ...
      throw new BadRequestException('Failed to remove penghargaan data: ' + error.message);
    }
  }
}
