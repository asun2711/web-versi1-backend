...
@Controller('api/datarawatjalan')
export class RawatJalanController {
  constructor(private readonly service: RawatJalanService) {}
  ...
  @Post()
  @UseInterceptors(
    FileInterceptor('gambarrawatjalan', {
      storage: diskStorage({
        destination: join(process.cwd(), 'uploads/rawatjalan'),
  ...
  async create(@Body() data: any, @UploadedFile() file: any) {
    try {
      if (file) {
        const filePath = join(process.cwd(), 'uploads/rawatjalan', file.filename);
      ...
      throw new BadRequestException('Failed to save rawatjalan data: ' + error.message);
    }
  }
  ...
  async update(@Param('id') id: string, @Body() data: any, @UploadedFile() file: any) {
    try {
      ...
      throw new BadRequestException('Failed to update rawatjalan data: ' + error.message);
    }
  }
  ...
  async remove(@Param('id') id: string) {
    try {
      ...
      throw new BadRequestException('Failed to remove rawatjalan data: ' + error.message);
    }
  }
}
