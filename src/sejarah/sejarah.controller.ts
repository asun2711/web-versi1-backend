...
export class SejarahController {
  constructor(private readonly service: SejarahService) {}
  ...
  async create(@Body() data: any, @UploadedFile() file: any) {
    try {
      if (file) {
        const filePath = join(process.cwd(), 'uploads/sejarah', file.filename);
  ...
  async remove(@Param('id') id: string) {
    try {
      ...
      throw new BadRequestException('Failed to remove sejarah data: ' + error.message);
    }
  }
}
