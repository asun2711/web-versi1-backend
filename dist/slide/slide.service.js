...
let SlideService = class SlideService {
    ...
    async update(id, data) {
        ...
        if (!slide) {
            throw new common_1.BadRequestException('Slide not found');
        }
        if (data.gambarslide && slide.gambarslide) {
            const oldFilePath = (0, path_1.join)(__dirname, '..', '..', 'uploads', 'slide', slide.gambarslide);
            try {
    ...
    async remove(id) {
        ...
        if (slide.gambarslide) {
            const filepath = (0, path_1.join)(__dirname, '..', '..', 'uploads', 'slide', slide.gambarslide);
            try {
